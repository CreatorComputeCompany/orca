...
export type NonTransplantableCookieStore = Pick<CookieClearStore, 'get' | 'remove'>
...
export async function hasNonTransplantableCookies(
  store: Pick<NonTransplantableCookieStore, 'get'>
): Promise<boolean> {
  return (await store.get({})).some((cookie) =>
