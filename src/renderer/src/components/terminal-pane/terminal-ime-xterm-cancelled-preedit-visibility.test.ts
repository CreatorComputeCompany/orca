...
/**
 * Cancelling a composition must also take the preedit overlay off the screen.
 *
 * Reported for macOS Cangjie (#11951): compose until the preedit shows 尸, press Backspace before
 * committing a candidate, and 尸 stays painted over the terminal. Backspacing the last radical
 * makes the IME's marked text empty, which Chromium reports either as a `compositionupdate` with
 * empty data or — when the composition never produced a candidate — as nothing more than the
 * deleting `input` event. Neither path reaches `compositionend`, and xterm only ever dropped the
 * overlay's `active` class there, so the box kept whatever glyph it was last given.
 *
 * The assertions are on what the user sees: the `active` class CSS keys `display: block` off, the
 * text inside the box, and the cells it therefore covers. `onData` is asserted too, but only as a
 * guard — it is already empty on the broken build, which is why the existing cancellation coverage
 * in `terminal-ime-xterm-composition-cancel.test.ts` cannot catch this.
 *
 * happy-dom performs no layout, so the cell size is supplied and the covered footprint is derived
 * from the overlay's own display state rather than measured.
 */
...
/** Everything that decides whether the user still sees a preedit box. */
function displayedPreedit(view: HTMLElement): {
  ...
  // Without `active` the overlay is `display: none`, so it covers no cells whatever it still holds.
  return { coveredCells: shown ? preedit.length : 0, preedit, shown }
}
